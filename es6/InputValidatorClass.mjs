export class InputValidator {
    constructor(initialValues) {
        ...
        this.DEFAULT_ERROR_MESSAGES = {
            additionalContributionDollar: "Additional contribution dollars must be greater than or equal to zero.",
            additionalContributionPercent: "Additional contribution percentage must be between 0 and 100.",
            annualIncome: "Annual income must be between [annualIncomeMin] and [annualIncomeMax].",
            annualContributionLimit: "Annual contribution exceeds IRS limit of [annualContributionLimitAmt].",
            contributionsOverAnnualIncome: "Total contributions exceed annual income.",
            contributionDollar: "Contribution dollars must be greater than or equal to zero.",
            contributionPercent: "Contribution percentage must be between 0 and 100%.",
            currentAge: "Age must be between [minimumAge] and [retirementAge]",
            currentSavings: "Current savings cannot be negative.",
            employerMatchPercent: "Employer matching percentage must be between 0 and 100%.",
            employerMaxMatchPercent: "Employer maximum matching percentage must be between 0 and 100%.",
            endOfRetirementAge: "End of retirement age must be greater than retirement age.",
        ...
        this.convertToNumberFriendlyFormat = (num, toUpperCase, threshold) => {
            ...
            const cap = threshold !== null && threshold !== void 0 ? threshold : 1000;
            if (num < cap)
        ...
        this.replaceAllInString = (str, replacements, input) => {
            ...
            return placeholders.reduce((result, placeholder) => {
                ...
                let replacement = (_a = input === null || input === void 0 ? void 0 : input[placeholderKey]) !== null && _a !== void 0 ? _a : replacements === null || replacements === void 0 ? void 0 : replacements[placeholderKey];
         ...
         */
        this.replaceAllInErrorMessages = (errorMsgs) => Object.entries(errorMsgs !== null && errorMsgs !== void 0 ? errorMsgs : {}).reduce((msgs, [key, value]) => {
            const acc = msgs;
            acc[key] =
    ...
    getAnnualContributionLimit(age) {
        ...
        else if (limits.annual401kLimit &&
    ...
    createErrorMessages(input) {
        var _a, _b, _c, _d;
        const { annualIncomeLimits = [1000, 10000000], errorMsgs } = this.initialValues;
        const { currentAge } = input;
        const annualContributionLimitStr = (_b = (_a = this.DEFAULT_ERROR_MESSAGES) === null || _a === void 0 ? void 0 : _a.annualContributionLimit) !== null && _b !== void 0 ? _b : "";
        const annualContributionLimitAmt = "$" + this.getAnnualContributionLimit(currentAge).toLocaleString();
        // Convert annual income limits to a more user-friendly format
        const annualIncomeStr = (_d = (_c = this.DEFAULT_ERROR_MESSAGES) === null || _c === void 0 ? void 0 : _c.annualIncome) !== null && _d !== void 0 ? _d : "";
        const annualIncomeMin = this.convertToNumberFriendlyFormat(annualIncomeLimits[0], true);
        const annualIncomeMax = this.convertToNumberFriendlyFormat(annualIncomeLimits[1], true);
        return Object.assign(Object.assign(Object.assign({}, this.replaceAllInErrorMessages(this.DEFAULT_ERROR_MESSAGES)), {
            annualContributionLimit: this.replaceAllInString(annualContributionLimitStr, {
    ...
    validateInput(input, currentYearsAdditionalContribution, currentYearsContribution, currentYearsEmployerMatch, errors = {}, errorMessages = {}) {
        const { annualIncomeLimits = [1000, 10000000], endOfRetirementAge = 95, minimumAge = 16, } = this.initialValues;
        const { additionalContributionDollar = 0, additionalContributionPercent = 0, annualIncome, contributionDollar = 0, contributionPercent = 0, currentAge, currentSavings, employerMatchPercent = 0, employerMaxMatchPercent = 0, retirementAge, } = input;
        // Validate if the annual contribution limit exceeds this year's IRS limits
        if (currentYearsAdditionalContribution +
        ...
        if (currentYearsAdditionalContribution + currentYearsContribution >
            annualIncome) {
